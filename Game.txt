...
function createCameraControls(){
  ...
  zoomInBtn.title = '放大';
  zoomInBtn.addEventListener('click', ()=>{ if(interactionLocked) return; zoomCamera(1.18); });
  const zoomOutBtn = document.createElement('button');
  zoomOutBtn.type='button';
  zoomOutBtn.textContent = '−';
  zoomOutBtn.title = '缩小';
  zoomOutBtn.addEventListener('click', ()=>{ if(interactionLocked) return; zoomCamera(0.86); });
