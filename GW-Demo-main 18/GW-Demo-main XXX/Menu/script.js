...
const stageCatalog = {
  ...
  bloodTowerPlan: {
    ...
    size: '18 × 26',
    narrative: [
      '追踪赫雷西的血液采集路线后，小队来到了代号“血楼计划”的多层封锁区。',
      '内部划分为三道可摧毁墙体，敌人分波把守，并伴随愈发浓烈的血雾侵蚀。',
    ],
    brief: [
      '地图 18×26，存在两块大面积不可通行空洞：左上 (1-13, 8-12) 与右上 (6-18, 18-21)。',
      '初始敌人：雏形赫雷西成员×2、法形赫雷西成员×1、刺形赫雷西成员×1；击败后依次触发三道墙体的破坏与增援。',
      '可摧毁墙体①：列 21 行 1-5，摧毁后两回合开启血雾区（行 6-13，列 19-23），并生成恢复格 (3,18)。',
      '可摧毁墙体②：行 13 列 13-17，摧毁后两回合开启血雾区（行 7-13，列 13-18），并生成恢复格 (16,9)。',
      '可摧毁墙体③：行 13 列 1-7，摧毁后两回合开启血雾区（行 6-13，列 2-11），随后触发赫雷西成员B 的对峙剧情。',
      '掩体：纵列 (3-5,6) 与纵列 (1-7,9) 提供遮蔽；墙体初始阻断所有位移技能。',
      '血雾区域每回合对滞留单位造成 50 HP / 50 SP 以及 10 层流血与怨念，需迅速脱离。',
    ],
    map: (() => {
      const rows = 18;
      const cols = 26;
      const voids = new Set();
      const addVoid = (top, left, bottom, right) => {
      ...
      const cover = [];
      for (let r = 3; r <= 5; r += 1) cover.push({ row: r, col: 6 });
      for (let r = 1; r <= 7; r += 1) cover.push({ row: r, col: 9 });
      for (let r = 1; r <= 5; r += 1) cover.push({ row: r, col: 21 });
      for (let c = 13; c <= 17; c += 1) cover.push({ row: 13, col: c });
      for (let c = 1; c <= 7; c += 1) cover.push({ row: 13, col: c });

      const players = [
        { row: 16, col: 23, label: 'Da', type: 'player', tone: 'dario' },
        { row: 16, col: 24, label: 'Ad', type: 'player', tone: 'adora' },
        { row: 16, col: 25, label: 'Ka', type: 'player', tone: 'karma' },
      ];

      const enemies = [
        { row: 3, col: 23, label: '雏', type: 'enemy' },
        { row: 3, col: 25, label: '雏', type: 'enemy' },
        { row: 5, col: 24, label: '法', type: 'enemy' },
        { row: 18, col: 24, label: '刺', type: 'enemy' },
      ];

      return { rows, cols, voids, cover, players, enemies };
    })(),
    enemies: [
      {
        name: '雏形赫雷西成员',
        icon: '🩸',
        rank: '普通 / 等级 25',
        summary: 'HP 150 · SP 70（SP 归零：失控 1 回合、-1 步；眩晕期间所受伤害 ×2，结束后恢复 70 SP）',
        threat: 'enemy',
        skills: [
          { name: '被动：忠臣的信仰', detail: '每回合开始时恢复 10 SP。' },
          { name: '被动：Gift', detail: '受到攻击时有 50% 几率将伤害减半。' },
          { name: '被动：强化身体', detail: '每次攻击后自增 20% 伤害，每次受击时所受伤害 -20%。' },
          { name: '被动：接受神的指示', detail: '对拥有“邪教目标”状态的角色会追加特殊应对。' },
          { name: '干扰者死（1 步）', detail: '前方 1 格斩击，造成 15 HP 与 15 SP，并附加 1 层流血；若目标带有“邪教目标”，再追击一次同名技能。出现概率 80%。' },
          { name: '追上（2 步）', detail: '选择周围任意 3 格之一瞬移并消耗 5 SP；若以自身为中心 3×3 内存在“邪教目标”敌方单位，额外回复自身 10 HP 与 5 SP。出现概率 40%。' },
          { name: '献祭（2 步）', detail: '牺牲 20 HP 获得 1 层暴力，并为最近的敌方单位施加 1 层“邪教目标”。出现概率 25%。' },
          { name: '讨回公道！（3 步）', detail: '牺牲 35 HP，向前 2 格连续抓挠 4 次，每次造成 10 HP 与 5 SP 并叠加 1 层流血；若目标拥有“邪教目标”，再追加一次整套攻击。出现概率 10%。' },
        ],
      },
      {
        name: '法形赫雷西成员',
        icon: '🕯️',
        rank: '普通 / 等级 25',
        summary: 'HP 100 · SP 90（SP 归零：失控 1 回合、-1 步；眩晕期间所受伤害 ×2，结束后恢复 90 SP）',
        threat: 'enemy',
        skills: [
          { name: '被动：忠臣的信仰', detail: '每回合开始时恢复 10 SP。' },
          { name: '被动：Gift', detail: '受到攻击时有 50% 几率将伤害减半。' },
          { name: '被动：强化身体', detail: '每次攻击后自增 20% 伤害，每次受击时所受伤害 -20%。' },
          { name: '被动：接受神的指示', detail: '对拥有“邪教目标”状态的角色会追加特殊应对。' },
          { name: '魔音影响（1 步）', detail: '以自身为中心 5×5 范围所有敌方单位受到 5 HP 与 25 SP，并附加 1 层怨念；若命中“邪教目标”，同范围内友军恢复 15 HP 与 15 SP。出现概率 80%。' },
          { name: '追上（2 步）', detail: '选择周围任意 3 格之一瞬移并消耗 5 SP；若以自身为中心 3×3 内存在“邪教目标”敌方单位，额外回复自身 10 HP 与 5 SP。出现概率 40%。' },
          { name: '献祭（2 步）', detail: '牺牲 20 HP，为任意友方单位提供 1 层暴力，并给最近的敌方单位施加 1 层“邪教目标”。出现概率 25%。' },
          { name: '毫无尊严（3 步）', detail: '牺牲 35 HP，以自身为中心 5×5 范围所有敌方单位 -25 SP，并附加 1 层一级脆弱（当回合受到伤害 +15%，回合结束 -1 层）；若命中“邪教目标”，同范围友军恢复 15 HP 与 15 SP。出现概率 10%。' },
        ],
      },
      {
        name: '刺形赫雷西成员',
        icon: '🗡️',
        rank: '普通 / 等级 25',
        summary: 'HP 50 · SP 100（SP 归零：失控 1 回合、-1 步；眩晕期间所受伤害 ×2，结束后恢复 100 SP）',
        threat: 'enemy',
        skills: [
          { name: '被动：忠臣的信仰', detail: '每回合开始时恢复 10 SP。' },
          { name: '被动：隐 Gift', detail: '初始隐身，未造成或受到伤害时保持隐身；若 3 回合无战斗互动会重新隐身。' },
          { name: '被动：刺形三角', detail: '无视所有减伤与防御机制。' },
          { name: '被动：接受神的指示', detail: '对拥有“邪教目标”状态的角色会追加特殊应对。' },
          { name: '割喉（2 步）', detail: '前方 1 格突刺造成 20 HP 与 5 SP；若目标带有“邪教目标”伤害提升 25%。出现概率 80%。' },
          { name: '暗袭（2 步）', detail: '以自身为中心 5×5 任意格位移；若相邻格内存在“邪教目标”，会追击最近目标并立即发动一次割喉。出现概率 50%。' },
          { name: '献祭（2 步）', detail: '牺牲 10 HP 获得 1 层灵活，并为最近敌方单位施加 1 层“邪教目标”。出现概率 25%。' },
          { name: '血溅当场（3 步）', detail: '牺牲 30 HP 对前方 1 格目标造成 45 HP；若目标有“邪教目标”，额外造成 10 HP 与 5 SP，并为自身叠 1 层灵活。出现概率 15%。' },
        ],
      },
      {
        name: '赫雷西初代精英成员',
        icon: '🛡️',
        rank: '精英 / 等级 25',
        summary: 'HP 200 · SP 50（SP 归零：失控 1 回合、-1 步；眩晕需累积 2 层眩晕叠加）',
        threat: 'elite',
        skills: [
          { name: '被动：忠臣的信仰', detail: '每回合开始时恢复 10 SP。' },
          { name: '被动：压迫巡逻', detail: '存活在场时，每回合额外获得 1 步。' },
          { name: '被动：血污蔓延', detail: '攻击命中的格子 2 回合内成为血污格（敌方停留时额外承受 5 HP 与 5 SP，并叠 1 层流血）。' },
          { name: '被动：接受神的指示', detail: '对拥有“邪教目标”状态的角色会追加特殊应对。' },
          { name: '异臂（2 步）', detail: '向前 2 格横扫，命中单位受到 15 HP 与 5 SP，并叠 1 层流血；若命中“邪教目标”，自身获得 1 层暴力。出现概率 80%。' },
          { name: '重锤（2 步）', detail: '以自身为中心 5×5 范围造成 20 HP 与 5 SP，并叠 1 层流血；若范围内至少两名敌人带有“邪教目标”，追加 1 层一级脆弱。出现概率 50%。' },
          { name: '献祭（2 步）', detail: '牺牲 10 HP，为自身叠 1 层暴力，并给最近的敌方单位施加 1 层“邪教目标”。出现概率 25%。' },
          { name: '爆锤（3 步，多阶段）', detail: '先以 3×3 范围造成 15 HP 并叠流血，再以 3×3 范围造成 15 HP 与 5 SP，最后以 5×5 范围造成 20 HP 与 5 SP 并叠流血；若命中“邪教目标”，自身获得 1 层暴力。出现概率 15%。' },
        ],
      },
      {
        name: '组装型进阶赫雷西成员（成员B）',
        icon: '🩸',
        rank: '小 Boss / 等级 25',
        summary: 'HP 250 · SP 90（SP 归零：失控 1 回合、-1 步；眩晕需累积 3 层眩晕叠加；嗜血之握无法斩杀，仅能造成 80 HP）',
        threat: 'boss',
        skills: [
          { name: '被动：忠臣的信仰', detail: '每回合开始时恢复 15 SP。' },
          { name: '被动：战场枢纽', detail: '存活时额外获得 1 步，并在 7×7 范围内存在友军时每回合回复 5% HP 与 5 SP。' },
          { name: '被动：传递神的指示', detail: '每次攻击有 35% 几率赋予目标 1 层“邪教目标”。' },
          { name: '以神明之名：“祝福”（2 步）', detail: '7×7 范围内所有友军获得 1 层暴力（除自身外至少有 1 名友军时才会使用）。出现概率 40%。' },
          { name: '以神明之名：“关怀”（2 步）', detail: '7×7 范围内所有友军（含自身）恢复 25 HP 与 10 SP（除自身外至少有 1 名友军时才会使用）。出现概率 40%。' },
          { name: '以神明之名：“自由”（3 步）', detail: '清除 7×7 范围内友军所有负面效果（当范围内存在负面效果时才会使用）。出现概率 40%。' },
          { name: '协助我们！（3 步）', detail: '在最近的空格生成 1 名雏形赫雷西成员。出现概率 40%。' },
          { name: '辅助我们！（3 步）', detail: '在最近的空格生成 1 名法形赫雷西成员。出现概率 40%。' },
          { name: '暗杀令（2 步）', detail: '在最近的空格生成 1 名半血刺形赫雷西成员。出现概率 40%。' },
          { name: '以神明之名：“清除”（2 步）', detail: '对前方 3×3 敌方单位造成 15 HP 与 15 SP，并引爆所有敌人身上的“邪教目标”（每层 10 HP / 10 SP）。出现概率 60%。' },
        ],
      },
    ],
  },
