...
// === Boss战 BGM 控制（进入Boss时开始循环，结束时停止） ===
...
if (storySkipButton) {
  storySkipButton.addEventListener('click', (event) => { window.GWExpectBossOnSkip = true;
    event.stopPropagation();
...
// ===== Boss Overlay Helpers (inline srcdoc) =====
...
(function setupInlineBossBridge(){
  // Exit button
  const exitBtn = document.querySelector('.boss-exit');
  if(exitBtn){ exitBtn.addEventListener('click', closeBossOverlay); }
...
// Fallback: if user clicked Skip and the story overlay hides, open boss (inline) immediately.
(function InlineSkipObserver(){
  try{
    const overlayEl = document.querySelector('.story-overlay');
    if(!overlayEl) return;
    const mo = new MutationObserver(() => {
      if (window.GWExpectBossOnSkip && (overlayEl.getAttribute('aria-hidden')==='true' || !overlayEl.classList.contains('active'))) {
        try{ openBossOverlay(); }catch(e){}
        window.GWExpectBossOnSkip = false;
      }
    });
    mo.observe(overlayEl, { attributes:true, attributeFilter:['class','aria-hidden'] });
  }catch(e){}
})();
