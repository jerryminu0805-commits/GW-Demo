...
const stageCatalog = {
  ...
  bloodTower: {
    ...
    size: '18 × 26',
    narrative: [
      '在迷雾深处的血色高楼中，赫雷西教团布下了层层机关与精锐守卫。',
      '三人组必须突破重重障碍，揭开教团的核心秘密。',
    ],
    brief: [
      '地图 18×26，包含多个区域与可摧毁墙体机关。',
      '我方：Dario (16, 23)、Adora (16, 24)、Karma (16, 25)，全员等级 25。',
      '初始敌方：雏形赫雷西成员 × 3、法形赫雷西成员 × 1、刺形赫雷西成员 × 1。',
      '特殊机制：摧毁墙体后会生成新的敌人并触发血雾区域（每回合 -50 HP、-50 SP、+10 层流血、+10 层怨念）。',
      '恢复格子：特定位置可一次性恢复全部 HP、SP 并获得鸡血状态。',
      '最终 Boss：组装型进阶赫雷西成员（赫雷西成员 B）。',
    ],
    map: {
      rows: 18,
      cols: 26,
      voids: [
        // (6,21) 到 (18,21) 到 (18,18) 到 (6,18) 是空的
        // (1,8) 到 (1,12) 到 (13,12) 到 (13,8) 是空的
      ],
      cover: [
        { row: 6, col: 3 },
        { row: 6, col: 4 },
    ...
    enemies: [
      ...
      },
      {
        ...
        skills: [
          { name: '被动：忠臣的信仰', detail: '每回合开始回复 10 SP。' },
          { name: '被动：隐 Gift', detail: '一开始就隐身（无法看到此单位或点击，移动时镜头不会跟随此单位除非隐身被解除），若造成伤害或受到伤害则解除隐身，若 3 回合未受到伤害或造成伤害则重新隐身。' },
          { name: '被动：刺形三角', detail: '无视所有减伤机制或防御。' },
          { name: '被动：接受神的指示', detail: '对拥有"邪教目标"状态的角色将采取额外手段。' },
          { name: '割喉（2 步）', detail: '对前方 1 格的敌方单位划动匕首造成 20 HP 与 5 SP（若攻击对象有"邪教目标"此技能攻击上升 25%）。出现概率 80%。' },
          { name: '暗袭（2 步）', detail: '以自身为中心 5×5 以内可选择任何格子并移动（若在相邻内有任何一个敌方单位有"邪教目标"，则选择最近的有"邪教目标"敌方单位并追击一次割喉）。出现概率 50%。' },
          { name: '献祭（2 步）', detail: '牺牲自身 10 HP，给自己增加 1 层灵活 Buff，并为距离最近的敌方单位施加 1 层"邪教目标"。出现概率 25%。' },
          { name: '血溅当场（3 步）', detail: '牺牲自身 30 HP，用匕首插进前方 1 格的敌方单位的胸口造成 45 HP（若目标有"邪教目标"，则伤害额外增加 10 HP、5 SP，并给自己上 1 层灵活 Buff）。出现概率 15%。' },
        ],
      },
      {
        name: '赫雷西初代精英成员',
        icon: '⚔️',
        rank: '精英 / 等级 25',
        summary: 'HP 200 · SP 50（降至 0：失控 1 回合、-1 步，结束时恢复至 50，眩晕期间所受伤害 ×2）',
        threat: 'elite',
        skills: [
          { name: '被动：忠臣的信仰', detail: '每回合开始回复 10 SP。' },
          { name: '被动：额外行动', detail: '若存活在场，每回合额外获得 1 步。' },
          { name: '被动：血污蔓延', detail: '攻击到的格子会变成"血污格子"（在此类格子内的敌方单位受到 5 HP、5 SP 并叠 1 层流血）状态持续 2 回合，然后恢复普通格子。' },
          { name: '被动：接受神的指示', detail: '对拥有"邪教目标"状态的角色将采取额外手段。' },
          { name: '异臂（2 步）', detail: '向前方 2 格的所有敌方单位挥舞异变手臂造成 15 HP、5 SP 并附加 1 层流血（若攻击对象有"邪教目标"则给自己上 1 层暴力）。出现概率 80%。' },
          { name: '重锤（2 步）', detail: '以自身为中心 5×5 进行重锤，对所有敌方单位造成 20 HP、5 SP 并附加 1 层流血（若在此攻击范围内至少有 2 个敌方单位有"邪教目标"，则给所有在此攻击范围内的敌方单位上 1 层一级脆弱 Debuff）。出现概率 50%。' },
          { name: '献祭（2 步）', detail: '牺牲自身 10 HP，给自己增加 1 层暴力，并为距离最近的敌方单位施加 1 层"邪教目标"。出现概率 25%。' },
          { name: '爆锤（多阶段攻击）（3 步）', detail: '牺牲自身 30 HP，开始用力砸：以自身为中心 3×3 对所有敌方单位造成 15 HP 并上 1 层流血，再以自身为中心 3×3 砸地对所有敌方单位造成 15 HP、5 SP，最后蓄力用力一大砸以自身为中心 5×5 所有敌方单位造成 20 HP、5 SP 并附加 1 层流血（若击中目标有"邪教目标"，给自己上 1 层暴力 buff）。出现概率 15%。' },
        ],
      },
      {
        name: '组装型进阶赫雷西成员（赫雷西成员 B）',
        icon: '👤',
        rank: '小 Boss / 等级 25',
        summary: 'HP 250 · SP 90（降至 0：失控 1 回合、-1 步，结束时恢复至 90，眩晕期间所受伤害 ×2）',
        threat: 'miniboss',
        skills: [
          { name: '被动：忠臣的信仰', detail: '每回合开始回复 15 SP。' },
          { name: '被动：额外行动', detail: '若存活在场，每回合额外获得 1 步。' },
          { name: '被动：安抚灵魂', detail: '若有友方单位在自身 7×7 格子范围内，回复 5% 血量及 5 点 SP。' },
          { name: '被动：传递神的指示', detail: '每次攻击都有 35% 几率给敌方单位上"邪教目标"。' },
          { name: '以神明之名："祝福"（2 步）', detail: '7×7 格子内所有友方成员获得 1 层暴力 Buff（若场上至少有 1 个除自己以外的友方单位才会使用）。出现概率 40%。' },
          { name: '以神明之名："关怀"（2 步）', detail: '7×7 格子内所有友方成员（包括自己）恢复 25 HP 及 10 SP（若场上至少有 1 个除自己以外的友方单位才会使用）。出现概率 40%。' },
          { name: '以神明之名："自由"（3 步）', detail: '清除 7×7 格子内所有友方成员的所有负面效果/Debuff（若场上至少有 1 个除自己以外的友方单位有负面效果/Debuff 才会使用）。出现概率 40%。' },
          { name: '协助我们！（3 步）', detail: '在离自己最近的空格子里生成一个"雏形赫雷西成员"。出现概率 40%。' },
          { name: '辅助我们！（3 步）', detail: '在离自己最近的空格子里生成一个"法形赫雷西成员"。出现概率 40%。' },
          { name: '暗杀令（2 步）', detail: '在离自己最近的空格子里生成一个半血"刺形赫雷西成员"。出现概率 40%。' },
          { name: '以神明之名："清除"（2 步）', detail: '对前方 3×3 的格子里所有敌方单位造成 15 HP 及 15 SP，并引爆场上所有敌方单位身上的所有"邪教目标"（每层"邪教目标" 10 HP、10 SP）。出现概率 60%。' },
        ],
      },
    ],
  },
...
const stageStories = {
  ...
  bloodTower: [
    { type: 'narration', text: '在调查赫雷西教团的过程中，三人组发现了一座隐藏在迷雾深处的血色高楼。', audio: 'Tower.mp3', audioAction: 'play' },
    { speaker: 'Adora', text: '这里……就是赫雷西教团的核心据点吗？', portrait: 'AdoraWorried.png', position: 'center', characters: { Adora: { portrait: 'AdoraWorried.png', position: 'center' }, Dario: { portrait: 'DarioThinking.png', position: 'left' }, Karma: { portrait: 'KarmaAnnoyed.png', position: 'right' } } },
    { speaker: 'Dario', text: '看起来这次要面对真正的挑战了。', portrait: 'DarioThinking.png', position: 'left', characters: { Adora: { portrait: 'AdoraWorried.png', position: 'center' }, Dario: { portrait: 'DarioThinking.png', position: 'left' }, Karma: { portrait: 'KarmaAnnoyed.png', position: 'right' } } },
    { speaker: 'Karma', text: '管他的，干就完了！', portrait: 'KarmaYell.png', position: 'right', characters: { Adora: { portrait: 'AdoraWorried.png', position: 'center' }, Dario: { portrait: 'DarioThinking.png', position: 'left' }, Karma: { portrait: 'KarmaYell.png', position: 'right' } } },
    { type: 'narration', text: '三人踏入血楼，空气中弥漫着浓郁的血腥味与诡异的低语。' },
    { type: 'narration', text: '（进入战斗）' },
  ],
};
...
function finishStageStory(skipped = false) {
  ...
  setTimeout(() => {
    ...
    // Redirect to blood tower battle after bloodTower story (even if skipped)
    if (stageId === 'bloodTower') {
      setTimeout(() => {
...
function initStageBoard() {
  ...
  if (enterBtn) {
    enterBtn.addEventListener('click', () => {
      ...
      if (currentStageId === 'bloodTower') {
        // Navigate to Blood Tower battle
        if (bgmController && typeof bgmController.fadeOut === 'function') {
          bgmController.fadeOut(850);
        }
        startStageStory('bloodTower');
        return;
      }
